...
const LiquidBackground = () => {
  ...
  useEffect(() => {
    ...
    const { gl, ext, isWebGL2 } = webGLContext;
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (available_extensions) {
          for(let i=0; i<available_extensions.length; i++) {
              ...
              if (isWebGL2) {
                  if(e.includes('EXT_color_buffer_float')) {
                      extensions.format = {
                  ...
                  if (e.includes('OES_texture_half_float')) {
                      extensions.typeHalfFloat = (gl.getExtension(e) as OES_texture_half_float).HALF_FLOAT_OES;
                  }
              }
          }
      }
      ...
      if (!extensions.format) {
        const type = extensions.typeHalfFloat || gl.UNSIGNED_BYTE;
        extensions.format = {
          internal: gl.RGBA,
          format: gl.RGBA,
          type: type,
    ...
    function update() {
      ...
      gl.uniform2f(
        advectionProgram.uniforms['u_texelSize'],
        texelSize.x,
        texelSize.y
      );
      ...
      gl.uniform2f(
        divergenceProgram.uniforms['u_texelSize'],
        texelSize.x,
        texelSize.y
      );
      ...
      gl.uniform2f(
        pressureProgram.uniforms['u_texelSize'],
        texelSize.x,
        texelSize.y
      );
      ...
      gl.uniform2f(
        gradienSubtractProgram.uniforms['u_texelSize'],
        texelSize.x,
        texelSize.y
      );
      gl.uniform1i(
        gradienSubtractProgram.uniforms['u_pressure'],
        pressure.read.attach(0)
      );
      gl.uniform1i(
        gradienSubtractProgram.uniforms['u_velocity'],
